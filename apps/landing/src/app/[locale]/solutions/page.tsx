...
// Ingredient type keys for translations
const ingredientTypeKeys = ["vitamins", "minerals", "aminoAcids", "botanicals", "specialty"];

// Service keys for translations
const serviceKeys = [
  { id: "singleIngredient", icon: "package" },
  { id: "customPremix", icon: "beaker" },
  { id: "supplyChain", icon: "truck" },
  { id: "qualityDocs", icon: "document" },
  { id: "technicalSupport", icon: "support" },
  { id: "inventoryPrograms", icon: "calendar" },
];

// Customer type keys for translations
const customerTypeKeys = ["0", "1", "2", "3", "4", "5", "6", "7"];
    ...
    <div className="antialiased selection:bg-[#2A2118] selection:text-white text-neutral-900">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category, index) => (
              ...
              >
                <img
                  src={category.image}
                  alt={tHome(`categories.items.${category.id}.title`)}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <h3 className="text-white text-xl font-semibold tracking-tight">
                    {tHome(`categories.items.${category.id}.title`)}
                  </h3>
      ...
      <section className="relative py-16 sm:py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12 sm:mb-16">
            <span className="text-sm uppercase tracking-widest text-[#df7a4c] font-medium animate-on-scroll text-reveal stagger-1">
              {t("ingredientTypes.badge")}
            </span>
            <h2 className="mt-4 text-3xl sm:text-4xl md:text-5xl tracking-tight font-playfair animate-on-scroll text-reveal stagger-2">
              {t("ingredientTypes.title")}
            </h2>
            <p className="mt-4 text-neutral-600 max-w-2xl mx-auto animate-on-scroll text-reveal stagger-3">
              {t("ingredientTypes.description")}
            </p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {ingredientTypeKeys.map((typeKey, index) => (
              <div
                key={typeKey}
                className={`bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition animate-on-scroll card-reveal stagger-${(index % 5) + 1}`}
              >
                <h3 className="font-semibold text-lg text-[#2A2118] mb-4">{t(`ingredientTypes.${typeKey}.title`)}</h3>
                <ul className="space-y-2">
                  {(t.raw(`ingredientTypes.${typeKey}.items`) as string[]).map((item) => (
              ...
              <div className="mt-8 space-y-4">
                {premixSolutions.map((premix, index) => (
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-semibold text-white">{tHome(`categories.premix.items.${premix.id}`)}</h4>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="hidden lg:block">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-6 animate-on-scroll card-reveal stagger-1">
                  ...
                  <p className="text-neutral-400 text-sm mt-1">{t("premix.stats.singleIngredients")}</p>
                </div>
                <div className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-6 animate-on-scroll card-reveal stagger-2">
                  ...
                  <p className="text-neutral-400 text-sm mt-1">{t("premix.stats.premixBlends")}</p>
                </div>
                <div className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-6 animate-on-scroll card-reveal stagger-3">
                  ...
                  <p className="text-neutral-400 text-sm mt-1">{t("premix.stats.documentation")}</p>
                </div>
                <div className="bg-white/10 backdrop-blur border border-white/20 rounded-2xl p-6 animate-on-scroll card-reveal stagger-4">
                  ...
                  <p className="text-neutral-400 text-sm mt-1">{t("premix.stats.quoteResponse")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceKeys.map((service, index) => (
              ...
              >
                ...
                <h3 className="font-semibold text-lg text-[#2A2118]">{t(`services.items.${service.id}.title`)}</h3>
                <p className="mt-3 text-sm text-neutral-600 leading-relaxed">{t(`services.items.${service.id}.description`)}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {qualityPillars.map((pillar, index) => (
              ...
              >
                ...
                <h3 className="font-semibold text-[#2A2118] text-lg">{tHome(`quality.pillars.${pillar.id}.title`)}</h3>
                <p className="mt-3 text-sm text-neutral-600 leading-relaxed">{tHome(`quality.pillars.${pillar.id}.description`)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-16 sm:py-24 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4">
            {customerTypeKeys.map((typeKey, index) => (
              <span
                key={typeKey}
